...
const createBook = async function (req, res) {
    try {
        ...
        if(!isValidEntry(excerpt) || excerptRegex.test(excerpt)) return res.status(400).send({status : false , message : "Excerpt is not given or Not Invalid."})


        let allreadyBook = bookModel.findOne({$or : [{title : title} , {ISBN : ISBN}]})
...
const getBooks = async function (req, res) {
    ...
    try {
        ...
        let allBook = await bookModel.find(findObj).sort({ title: 1 }).select({ _id: 1, title: 1, excerpt: 1, userId: 1, category: 1,  reviews: 1 , releasedAt : 1 })
